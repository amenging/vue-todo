      ...
      if (this.listData) {
        ...
        if (beforeOnline && !data.online) {
          ...
          .then(data => {
            this.waiting = false
            if (data.code == 0) {
              this.todos[this.title].list_id = data.list_id
              const lists = this.todos[this.title]
              for (var i in lists) {
                Cloud.addNewItem(this.todos[this.title].list_id, lists.content)
              }
